...
function get_free_display() {
  ...
  [ ! -s /tmp/netstatX11.log ] && echo -n "-- INFO: Emtpy file /tmp/netstatX11.log" 1>&3
  ...
  for find_display_num in ${arrayDispNums[@]}; do
    ...
    if [ -s /tmp/netstatX11.log ]; then
      while read read_disp_num ; do
        if [ "${read_disp_num}" = "${find_display_num}" ]; then
          echo -n "-- WARN: DISPLAY=:${find_display_num} is taken, searching for another..." 1>&3
          selected_disp_num="-1"
          break
        elif [ "${selected_disp_num}" = "-1" ]; then
          selected_disp_num="${find_display_num}"
          echo -n "-- INFO: Possible free DISPLAY=:${find_display_num}" 1>&3
    ...
    if [ "${selected_disp_num}" != "-1" ]; then
      ...
      if xsetroot -cursor_name left_ptr -fg white -bg black > /dev/null 2>&1; then
        echo -n "-- WARN: DISPLAY=:${find_display_num} is already being used, skip it..." 1>&3
        selected_disp_num="-1"
      fi
    fi
    if [ "${selected_disp_num}" != "-1" ]; then
      ...
      echo -n "-- ERROR: Entered in an infinite loop at $0 after netstat" 1>&2 1>&3
      selected_disp_num="-1"
      break
    fi
  done
...
function start_xvfb() {
  ...
  echo -n "-- INFO: Will try to start Xvfb at DISPLAY=${DISPLAY}" 1>&3
  # if DEBUG = true ...
  ...
  Xvfb ${DISPLAY} -screen 0 1900x1880x16 \
...
while true ; do
    ...
    if ! start_xvfb; then
      echo -n "-- WARN: start_xvfb() failed!" 1>&3
    fi
    if timeout --foreground "${WAIT_FOREGROUND_RETRY}" wait-xvfb.sh &> "${LOGS_DIR}/wait-xvfb-stdout.log"; then
      break
    else
      echo -n "-- WARN: wait-xvfb.sh failed! for DISPLAY=${DISPLAY}" 1>&3
    fi
    if [ ${i} -gt ${MAX_DISPLAY_SEARCH} ]; then
      echo -n "-- ERROR: Failed to start Xvfb at $0 after many retries." 1>&2 1>&3
      break
    fi
done 
